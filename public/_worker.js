  ...
  try {
    ...
    if (Array.isArray(manifest.icons)) {
      manifest.icons = manifest.icons.map((icon) => ({
        ...icon,
        src: rewrite(icon.src),
      }));
    }
    if (typeof manifest.start_url === "string") {
      manifest.start_url = rewrite(manifest.start_url);
    }
    if (typeof manifest.scope === "string") {
      manifest.scope = rewrite(manifest.scope);
    }
...
async function proxy(request, prefix, origin) {
  ...
  let response = await fetch(new Request(upstream.toString(), request));

  if (response.status === 404 && request.method === "GET" && upstreamPath !== "/") {
    const fallback = new URL("/", origin);
    fallback.search = incoming.search;
    response = await fetch(new Request(fallback.toString(), request));
  }
  ...
  if (contentType.includes("text/html")) {
    return new Response(rewriteHtml(await response.text(), prefix), {
